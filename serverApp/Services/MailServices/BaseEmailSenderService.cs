...
public class BaseEmailSenderService
{
    ...
    public async Task Send(string fromAddress, string emailPas, string toAddress, string host, int port, string title, string htmlBody)
    {
        ...
        {
            _logger.LogError("The mail format may be incorrect. FormatException: " + ex.Message);
        }
        catch (ArgumentException ex)
        {
            _logger.LogError("You may have entered an empty line with the mail. ArgumentException: " + ex.Message);
        }
        catch (Exception ex)
        {
